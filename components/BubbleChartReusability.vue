...
export default {
    ...
    methods: {
        build_traces(){
            ...
            //control scores
            if(this.current_collection != 'tools'){
                var scores = this.control_fair_scores['R']['fair_scores']
                var labels = this.control_fair_scores['R']['labels']
            ...
            //collection scores
            var scores = this.fair_scores['R']['fair_scores']
            var labels = this.fair_scores['R']['labels']
            for (let i = 0, len = scores.length; i < len; i++){
                var item = scores[i]
                var trace = {
                    y:  Array(item.scores.length).fill(labels[item.indicator]), 
